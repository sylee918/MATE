   Subroutine Init_Particles(ptl)
      ...
      USE SET_VELOCITY_DIRECTION, only: cached_vel_dir
...
!            ptl(:,iR,iE,4) = radial_distance_range(iR)*sin_lat                ! Z
            ...
            ptl(:,iE,5) = cached_vel_dir(:,1) * energy_to_speed       ! Vx
            ptl(:,iE,6) = cached_vel_dir(:,2) * energy_to_speed       ! Vy
            ptl(:,iE,7) = cached_vel_dir(:,3) * energy_to_speed       ! Vz
      enddo
